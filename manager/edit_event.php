...
$id = intval($_GET['id']);
$q = $db->prepare('SELECT * FROM events WHERE id = ? AND school_id = ?');
...
$q = $db->prepare('UPDATE events SET building_name = ?, building_location = ?, building_image = ?, event_info = ? WHERE id = ? AND school_id = ?');
$q->execute(array($bname, $bloc, $img, $info, $id, $sid));
if ($bad == '') {
header('Location: dashboard.php');
exit;
}
...
<a href="dashboard.php" class="btn gold">Cancel</a>
</form>
</div>
